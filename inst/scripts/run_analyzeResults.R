...
# Now load the packages if they are installed
library(readxl)
library(ggplot2)
library(rstan)
library(bayesplot)
...
for(calibrationperiod in calibrationperiods){ 
    ...
    expected_file_path <- list.files(path = data_file_location, 
...
##############################################################################
  for (i in 1:length(fitting_index)) {
    ...
    plot <- ggplot(data, aes(x = Date)) +
      ...
      geom_line(aes(y = median), color = "red", linewidth = 1, show.legend = FALSE) +
      geom_line(aes(y = LB), color = "black", linetype = "dashed", linewidth = 0.5, show.legend = FALSE) +
      geom_line(aes(y = UB), color = "black", linetype = "dashed", linewidth = 0.5, show.legend = FALSE) +
      labs(title = '', x = datetype, y = series_cases[i]) + 
      theme_minimal()
    
    # Add the vertical line conditionally
    if (forecastinghorizon > 0) {
      plot <- plot + geom_vline(xintercept = calibrationperiod - 1, linetype = "dotted", linewidth = 1, color = "blue", show.legend = FALSE)
    }
    ...
    pdf_filename <- file.path(
      full_folder_path, 
      sprintf(
        "Forecast-%s-%s-%s-%s-cal-%d-fcst-%d.pdf",
        model_name,               # Assuming scalar
        caddisease,            # Extract the first element if it's a vector
        errorstructure[errstrc],  # Extract the relevant element (use correct index)
        series_cases[i],          # Assuming scalar, or adjust if it's a vector
        calibrationperiod,        # Assuming scalar
        forecastinghorizon        # Assuming scalar
      )
